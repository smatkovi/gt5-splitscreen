...
#        patch_arcade_unlock.py (all cars and all courses in the arcade lists),
#        patch_race_log_w6.py with MOD_WINDOW_MAX (default 3), patch_hud_quad.py (quadrant HUD
#        incl. the per-window speed readout), patch_countdown_split.py (per-window start countdown),
...
python3 "$BASE/patch_arcade_unlock.py" "$SRC/arcade"
python3 "$BASE/patch_race_log_w6.py" "$SRC/race"
python3 "$BASE/patch_hud_quad.py" "$SRC/race"
python3 "$BASE/patch_countdown_split.py" "$SRC/race"
python3 - "$SRC" <<'PYEOF'
import sys, pathlib
