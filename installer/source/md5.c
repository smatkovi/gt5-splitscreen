/* Compact MD5 (RFC 1321). Used only to identify which EBOOT.BIN is installed,
   never for anything security relevant. */
#include "md5.h"
#include <string.h>

#define ROL(x, c) (((x) << (c)) | ((x) >> (32 - (c))))

static const unsigned int K[64] = {
    0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee, 0xf57c0faf, 0x4787c62a,
    0xa8304613, 0xfd469501, 0x698098d8, 0x8b44f7af, 0xffff5bb1, 0x895cd7be,
    0x6b901122, 0xfd987193, 0xa679438e, 0x49b40821, 0xf61e2562, 0xc040b340,
    0x265e5a51, 0xe9b6c7aa, 0xd62f105d, 0x02441453, 0xd8a1e681, 0xe7d3fbc8,
    0x21e1cde6, 0xc33707d6, 0xf4d50d87, 0x455a14ed, 0xa9e3e905, 0xfcefa3f8,
    0x676f02d9, 0x8d2a4c8a, 0xfffa3942, 0x8771f681, 0x6d9d6122, 0xfde5380c,
    0xa4beea44, 0x4bdecfa9, 0xf6bb4b60, 0xbebfbc70, 0x289b7ec6, 0xeaa127fa,
    0xd4ef3085, 0x04881d05, 0xd9d4d039, 0xe6db99e5, 0x1fa27cf8, 0xc4ac5665,
    0xf4292244, 0x432aff97, 0xab9423a7, 0xfc93a039, 0x655b59c3, 0x8f0ccc92,
    0xffeff47d, 0x85845dd1, 0x6fa87e4f, 0xfe2ce6e0, 0xa3014314, 0x4e0811a1,
    0xf7537e82, 0xbd3af235, 0x2ad7d2bb, 0xeb86d391
};

static const unsigned char R[64] = {
    7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
    5,  9, 14, 20, 5,  9, 14, 20, 5,  9, 14, 20, 5,  9, 14, 20,
    4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23,
    6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21
};

static void md5_block(md5_ctx *c, const unsigned char *p) {
    unsigned int m[16], a = c->h[0], b = c->h[1], d = c->h[2], e = c->h[3];
    int i;
    /* the PPU is big endian, MD5 wants little endian words */
    for (i = 0; i < 16; ++i)
        m[i] = (unsigned int)p[i*4] | ((unsigned int)p[i*4+1] << 8) |
               ((unsigned int)p[i*4+2] << 16) | ((unsigned int)p[i*4+3] << 24);

    for (i = 0; i < 64; ++i) {
        unsigned int f, g, t;
        if (i < 16)      { f = (b & d) | (~b & e);          g = i; }
        else if (i < 32) { f = (e & b) | (~e & d);          g = (5*i + 1) & 15; }
        else if (i < 48) { f = b ^ d ^ e;                   g = (3*i + 5) & 15; }
        else             { f = d ^ (b | ~e);                g = (7*i) & 15; }
        t = e; e = d; d = b;
        b = b + ROL(a + f + K[i] + m[g], R[i]);
        a = t;
    }
    c->h[0] += a; c->h[1] += b; c->h[2] += d; c->h[3] += e;
}

void md5_init(md5_ctx *c) {
    c->h[0] = 0x67452301; c->h[1] = 0xefcdab89;
    c->h[2] = 0x98badcfe; c->h[3] = 0x10325476;
    c->len = 0; c->n = 0;
}

void md5_update(md5_ctx *c, const void *data, unsigned int len) {
    const unsigned char *p = (const unsigned char *)data;
    c->len += len;
    while (len) {
        unsigned int take = 64 - c->n;
        if (take > len) take = len;
        memcpy(c->buf + c->n, p, take);
        c->n += take; p += take; len -= take;
        if (c->n == 64) { md5_block(c, c->buf); c->n = 0; }
    }
}

void md5_final(md5_ctx *c, unsigned char out[16]) {
    unsigned long long bits = c->len * 8ULL;
    unsigned char pad = 0x80;
    int i;
    md5_update(c, &pad, 1);
    pad = 0;
    while (c->n != 56) md5_update(c, &pad, 1);
    for (i = 0; i < 8; ++i) {
        unsigned char b = (unsigned char)(bits >> (8 * i));
        c->buf[56 + i] = b;
    }
    c->n = 64;
    md5_block(c, c->buf);
    for (i = 0; i < 4; ++i) {
        out[i*4+0] = (unsigned char)(c->h[i]);
        out[i*4+1] = (unsigned char)(c->h[i] >> 8);
        out[i*4+2] = (unsigned char)(c->h[i] >> 16);
        out[i*4+3] = (unsigned char)(c->h[i] >> 24);
    }
}

void md5_hex(const unsigned char in[16], char out[33]) {
    static const char *h = "0123456789abcdef";
    int i;
    for (i = 0; i < 16; ++i) { out[i*2] = h[in[i] >> 4]; out[i*2+1] = h[in[i] & 15]; }
    out[32] = 0;
}
