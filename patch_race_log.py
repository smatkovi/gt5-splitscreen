...
insert(loadutil, 'var loaded_num = ORG.getLoadedCarCount();',
       ...
       '+ " racersMax=" + ORG.getCurrentRaceParameter().racers_max.toString()); lu_dbg++;')
...
if 'log4p_poll' not in r:
    ...
    assert r.count(a1) == 1
    r = r.replace(a1, '''        // --- 4P split patch: the engine window manager only has two windows.
        // RaceOperator.window_max keeps the real player count (needed for car loading),
        // the engine side is clamped to 2 here.
        main::ORG.window_max = (main::RaceOperator.window_max > 2) ? 2 : main::RaceOperator.window_max;
